...
func (p *Processor) ProcessStruct(s Struct) error {
	...
	fmt.Fprintf(&buf, "\nrow := tx.QueryRow(`SELECT %s FROM ` + SessionTable + ` WHERE eid = ?`, eid)", scols.String())
	...
	buf.WriteString("\n}\n\n")
	fmt.Fprintf(&buf, "\nresult, err := tx.Exec(`INSERT INTO %s VALUES (%s)`, %s)", icols.String(), ipholders.String(), ifields.String())
	buf.WriteString("\nif err != nil {")
	buf.WriteString("\nreturn err")
	buf.WriteString("\n}\n")
