...
func generateModelFile(ctx *genctx, m Model) error {
	...
	if hasID {
		fmt.Fprintf(&buf, "\n\nfunc (%c *%s) Load(tx *sql.Tx, id string) (err error) {", varname, m.Name)
		buf.WriteString("\nif pdebug.Enabled {")
	...
	/*
		if l10nfields.Len() > 0 {
			...
			fmt.Fprintf(&buf, "\n\nfunc (v *%sL10N) LoadLocalizedFields(tx *sql.Tx) error {", m.Name)
...
func generateServiceFile(ctx *genctx, m Model) error {
	...
	if m.CreateRequest {
		...
		fmt.Fprintf(&buf, "\nfunc (v *%s) Create(ctx context.Context, tx *sql.Tx, vdb *db.%s, payload *model.Create%sRequest) (err error) {", svcname, m.Name, m.Name)
		buf.WriteString("\nif pdebug.Enabled {")
	...
	if hasID && m.Update {
		fmt.Fprintf(&buf, "\n\nfunc (v *%s) Update(tx *sql.Tx, vdb *db.%s) (err error) {", svcname, m.Name)
		buf.WriteString("\nif pdebug.Enabled {")
	...
	if hasID && m.UpdateRequest {
		fmt.Fprintf(&buf, "\n\nfunc (v *%s) UpdateFromPayload(ctx context.Context, tx *sql.Tx, payload *model.Update%sRequest) (err error) {", svcname, m.Name)
		buf.WriteString("\nif pdebug.Enabled {")
	...
	if hasL10N {
		fmt.Fprintf(&buf, "\n\nfunc (v *%s) ReplaceL10NStrings(tx *sql.Tx, m *model.%s, lang string) error {", svcname, m.Name)
		buf.WriteString("\nif pdebug.Enabled {")
	...
	if hasID && m.Delete {
		fmt.Fprintf(&buf, "\n\nfunc (v *%s) Delete(tx *sql.Tx, id string) error {", svcname)
		buf.WriteString("\nif pdebug.Enabled {")
