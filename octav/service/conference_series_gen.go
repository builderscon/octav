...
func (v *ConferenceSeriesSvc) Update(tx *db.Tx, vdb *db.ConferenceSeries, payload model.UpdateConferenceSeriesRequest) (err error) {
	if pdebug.Enabled {
	...
	key := `api.ConferenceSeries.` + vdb.EID
	c := Cache()
	c.Delete(key)
	if pdebug.Enabled {
...
func (v *ConferenceSeriesSvc) LoadList(tx *db.Tx, vdbl *db.ConferenceSeriesList, since string, limit int) error {
