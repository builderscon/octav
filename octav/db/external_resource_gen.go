...
const ExternalResourceStdSelectColumns = "external_resources.oid, external_resources.eid, external_resources.conference_id, external_resources.name, external_resources.url"
...
func (e *ExternalResource) Scan(scanner interface {
	...
	return scanner.Scan(&e.OID, &e.EID, &e.ConferenceID, &e.Name, &e.URL)
}
...
func init() {
	hooks = append(hooks, func() {
		...
		stmt.WriteString(ExternalResourceTable)
		stmt.WriteString(` SET eid = ?, conference_id = ?, name = ?, url = ? WHERE oid = ?`)
		...
		stmt.WriteString(`UPDATE `)
		stmt.WriteString(ExternalResourceTable)
		stmt.WriteString(` SET eid = ?, conference_id = ?, name = ?, url = ? WHERE eid = ?`)
...
func (e *ExternalResource) Create(tx *Tx, opts ...InsertOption) (err error) {
	...
	stmt.WriteString(ExternalResourceTable)
	stmt.WriteString(` (eid, conference_id, name, url) VALUES (?, ?, ?, ?)`)
	result, err := tx.Exec(stmt.String(), e.EID, e.ConferenceID, e.Name, e.URL)
	if err != nil {
		return err
	}
...
func (e ExternalResource) Update(tx *Tx) (err error) {
	...
	if e.OID != 0 {
		...
		_, err = tx.Stmt(stmt).Exec(e.EID, e.ConferenceID, e.Name, e.URL, e.OID)
		return err
	}
	if e.EID != "" {
		if pdebug.Enabled {
		...
		_, err = tx.Stmt(stmt).Exec(e.EID, e.ConferenceID, e.Name, e.URL, e.EID)
		return err
	}
