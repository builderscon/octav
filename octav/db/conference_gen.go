...
func (c *Conference) Scan(scanner interface {
	...
	return scanner.Scan(&c.OID, &c.EID, &c.CoverURL, &c.SeriesID, &c.Slug, &c.Status, &c.SubTitle, &c.Title, &c.TimetableAvailable, &c.Timezone, &c.CreatedBy, &c.CreatedOn, &c.ModifiedOn)
}
...
func init() {
	hooks = append(hooks, func() {
		...
		stmt.WriteString(ConferenceTable)
		stmt.WriteString(` SET eid = ?, cover_url = ?, series_id = ?, slug = ?, status = ?, sub_title = ?, title = ?, timetable_available = ?, timezone = ?, created_by = ? WHERE oid = ?`)
		...
		stmt.WriteString(`UPDATE `)
		stmt.WriteString(ConferenceTable)
		stmt.WriteString(` SET eid = ?, cover_url = ?, series_id = ?, slug = ?, status = ?, sub_title = ?, title = ?, timetable_available = ?, timezone = ?, created_by = ? WHERE eid = ?`)
...
func (c *Conference) Create(tx *Tx, opts ...InsertOption) (err error) {
	...
	stmt.WriteString(ConferenceTable)
	stmt.WriteString(` (eid, cover_url, series_id, slug, status, sub_title, title, timetable_available, timezone, created_by, created_on, modified_on) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
	result, err := tx.Exec(stmt.String(), c.EID, c.CoverURL, c.SeriesID, c.Slug, c.Status, c.SubTitle, c.Title, c.TimetableAvailable, c.Timezone, c.CreatedBy, c.CreatedOn, c.ModifiedOn)
	if err != nil {
		return err
	}
...
func (c Conference) Update(tx *Tx) error {
	if c.OID != 0 {
		...
		_, err = tx.Stmt(stmt).Exec(c.EID, c.CoverURL, c.SeriesID, c.Slug, c.Status, c.SubTitle, c.Title, c.TimetableAvailable, c.Timezone, c.CreatedBy, c.OID)
		return err
	}
	if c.EID != "" {
		...
		_, err = tx.Stmt(stmt).Exec(c.EID, c.CoverURL, c.SeriesID, c.Slug, c.Status, c.SubTitle, c.Title, c.TimetableAvailable, c.Timezone, c.CreatedBy, c.EID)
		return err
	}
