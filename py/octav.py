...
class Octav(object):
  ...
  def lookup_external_resource (self, id, lang=None, limit=None, since=None):
    try:
        payload = {}
        hdrs = {}
        if id is None:
        ...
        if since is not None:
            payload['since'] = since
        uri = '%s/v1/external_resource/lookup' % self.endpoint
        qs = urlencode(payload, True)
        if self.debug:
            print('GET %s?%s' % (uri, qs))
        res = self.http.request('GET', '%s?%s' % (uri, qs), headers=hdrs)
    ...
    try:
        ...
        for key in args:
            for p in patterns:
                if p.match(key):
                    payload[key] = args[key]
        uri = '%s/v1/external_resource/create' % self.endpoint
        hdrs = urllib3.util.make_headers(
            basic_auth='%s:%s' % (self.key, self.secret),
        )
        if self.debug:
    ...
    try:
        ...
        for key in args:
            for p in patterns:
                if p.match(key):
                    payload[key] = args[key]
        uri = '%s/v1/external_resource/update' % self.endpoint
        hdrs = urllib3.util.make_headers(
            basic_auth='%s:%s' % (self.key, self.secret),
        )
        if self.debug:
  ...
  def delete_external_resource (self, id, user_id):
    try:
        payload = {}
        hdrs = {}
        if id is None:
        ...
        if user_id is not None:
            payload['user_id'] = user_id
        uri = '%s/v1/external_resource/delete' % self.endpoint
        hdrs = urllib3.util.make_headers(
            basic_auth='%s:%s' % (self.key, self.secret),
        )
        if self.debug:
